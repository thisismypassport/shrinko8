...
def is_non_nested_short(node):
    # k_nested is still important due to shorthands being stoppable via end
    # (something shrinko8 doesn't generate but does preserve)
    return node.short and node.short is not k_nested

def get_orig_wspace(pre, post, allow_linebreaks, need_linebreak=False):
...
def output(root, minify_spaces=True, minify_lines=True, exclude_comments=True):
    """convert a root back to a string: either inserting as little whitespace as possible (under minify_spaces),
    ...
    def output_with_orig_wspace(token):
        ...
        if prev_token.endidx != token.idx or prev_token.modified or token.modified:
            ...
            wspace = get_orig_wspace(prev_token, token, allow_linebreaks, need_linebreak)
            ...
            if exclude_comments and token.children:
                ...
                prespace = get_orig_wspace(prev_token, token.children[0], allow_linebreaks)
                postspace = get_orig_wspace(token.children[-1], token, allow_linebreaks)
                for comment in token.children:
                    if comment.hint == CommentHint.keep:
    ...
    root.traverse_nodes(tokens=output_with_min_wspace if minify_spaces else output_with_orig_wspace,
