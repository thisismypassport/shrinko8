...
ni = split "end,else,elseif,until"

function nf(n, C)
  local r, Z, t = nr(n, true)
  ...
  local function d(e)
    ...
    if type(e) == "string" and E(b(e, 1)) and not M[e] then
      return e
    end
  ...
  local function y(r, o, i)
    ...
    if o then
      return function(e)
        local t = r(e)
        return t[o](t, I(b(e, n)))
      end, true, nil, function(e)
        local t = r(e)
        return t[o], pack(t, I(b(e, n)))
      end
    else
      return function(e)
        return r(e)(I(b(e, n)))
  ...
  local function E(t, l)
    local r = r[n]
    n += 1
    local n = sub(r, 1, -2)
    local n = z(n, 0, t, function()
      return e()
    end)
    if not n then
  ...
  local function m()
    ...
    if r[n] == ";" or G(r[n], ni) or h and h() then
      return function()
        return pack()
      end
    else
  ...
  c = function(e)
    ...
    while n <= #r and not G(r[n], ni) and not (h and h()) do
      local n, e = d(o)
      if n then
        add(o, n)
      end
  ...
  f = K and {_ENV = 0, _env = 0, _ = 0} or {_ENV = 0}
  local e = c()
  if n <= #r then
    o "unexpected end"
  end
  ...
  return function(n)
    local n = K and {_ENV = n, _env = n, _ = nl} or {_ENV = n}
    local n = e {[0] = n}
    if n then
      return I(n)
    end
...
function D(t, l)
  local n, e = 0, 0
  local o, r, t = V(t, function(t, i, r, o)
    if l == t then
      n, e = r, o
    end
  end)
...
function F(l, r, e)
  local t, n = 1, false
  local r, o, l = V(l, function(o, f, i, l)
...
function nw(n, r, o)
  ...
  W(n, r, o, function(r)
    ...
    elseif n == true then
      e = s[7]
    elseif type(n) ~= "string" or G(n, {"nil", "true", "false"}) then
      e = s[8]
    elseif M[n] then
      e = s[9]
    elseif not E(b(n, 1)) then
      e = s[10]
    elseif l[n] then
      e = s[11]
    end
...
function _draw()
  ...
  local function l(r)
    ...
    nw(l, 0, n)
    print(h, 0, n, s[4])
    if z >= 10 and r ~= false and not v then
      d(o * 4, n + t * 6)
    end
  end
  ...
  if H or g then
    t, e = F(g, 0, m)
    if e - m <= 20 and H then
      g, H = np(g, H)
      t, e = F(g, 0, m)
      if #H == 0 and not v then
        H = nil
      end
    end
  end
...
function Y(n, t, l)
  return nf(n, l)(t or e)
end

function nn(n, e)
  return Y("return " .. n, e, true)
end
...
function na(e, l)
  ...
  while true do
    ...
    if N and not d then
      ...
      if O and not d and not j then
        flip()
      end
      if not O and holdframe then
        holdframe()
      end
      j = false
    end
...
function nh(n)
  if G(n, {"i", "interrupt"}) then
    return N
  elseif G(n, {"f", "flip"}) then
    return O
  elseif G(n, {"r", "repl"}) then
    return K
  elseif G(n, {"mi", "max_items"}) then
    return T
  elseif G(n, {"h", "hex"}) then
    return U
  elseif G(n, {"cl", "colors"}) then
    return s
  elseif G(n, {"c", "code"}) then
    local n = {[0] = a}
    for e = 1, #w - 1 do
      n[e] = w[#w - e]
    end
    return n
  elseif G(n, {"cm", "compile"}) then
    return function(n)
      return n3(n)
    end
  elseif G(n, {"x", "exec"}) then
    return function(n, e)
      Y(n, e)
    end
  elseif G(n, {"v", "eval"}) then
    return function(n, e)
      return nn(n, e)
    end
  elseif G(n, {"p", "print"}) then
    return function(n, ...)
      e.print(f(n), ...)
    end
  elseif G(n, {"ts", "tostr"}) then
    return function(n)
      return f(n)
    end
  elseif G(n, {"rst", "reset"}) then
    run()
  elseif G(n, {"run"}) then
    n7(e)
  else
...
function n0(e)
  ...
  local n
  if G(e, {"i", "interrupt"}) then
    n = function(n)
      N = t(n)
    end
  elseif G(e, {"f", "flip"}) then
    n = function(n)
      O = t(n)
    end
  elseif G(e, {"r", "repl"}) then
    n = function(n)
      K = t(n)
    end
  elseif G(e, {"mi", "max_items"}) then
    n = function(n)
      T = tonum(n) or -1
    end
  elseif G(e, {"h", "hex"}) then
    n = function(n)
      U = t(n)
    end
  elseif G(e, {"cl", "colors"}) then
    n = function(n)
      s = n
    end
  else
...
function _update()
  ...
  elseif k(82) then
    if (r or not t(-1)) and B > 1 then
      f(-1)
    end
  elseif k(81) then
    if (r or not t(1)) and B < #w then
      f(1)
    end
  else
    local t = stat(31)
    ...
    elseif n == 203 then
      S = not S
      Z, Q = "shift now selects " .. (S and "punycase" or "symbols"), 40
    elseif k(74) then
      if r then
        c = 1
        e = true
  ...
  if n == 194 or n == 215 then
    if a ~= "" and a ~= R then
      ...
      Z = "press again to put in clipboard"
    else
      Z = ""
    end
  end
...
nc(1, function()
  assert(pack(nn "(function (...) return ... end)(1,2,nil,nil)").n == 4)
end)
nc(2, function()
  assert(nn "function() local temp, temp2 = {max(1,3)}, -20;return temp[1] + temp2; end" () == -17)
end)
printh "finished"
stop()
while true do
