...
if l=="\\"do n+=1local t=e(r,n)l=█[t]if t=="x"do t=tonum("0x"..sub(r,n+1,n+2))if(t)n+=2else o"bad hex escape"
l=chr(t)elseif g(t)do local i=n while(g(t)and n<i+3)n+=1t=e(r,n)
n-=1t=tonum(sub(r,i,n))if(not t or t>=256)o"bad decimal escape"
l=chr(t)elseif t=="z"do repeat n+=1t=e(r,n)until not f(t," \r	ᶜᵇ\n")if(t=="")o()
l=""n-=1elseif t==""do o()l=""end if(not l)o("bad escape: "..t)l=""
elseif l=="\n"do o"unterminated string"break end t..=l n+=1end if(n>#r)o("unterminated string",true)
return t,n+1end function W(t,n,l,r)if e(t,n)=="["do n+=1local r=n while(e(t,n)=="=")n+=1
local r="]"..sub(t,r,n-1).."]"local o=#r if e(t,n)=="["do n+=1if(e(t,n)=="\n")n+=1
...
elseif t=="-"and e(l,n+1)=="-"do n+=2if(e(l,n)=="[")r,n=W(l,n,d)
if(not r)while(n<=#l and e(l,n)~="\n")n+=1
if(c)a=true else add(o,true)
elseif g(t)or t=="."and g(e(l,n+1))do local u,a="0123456789",true if t=="0"and f(e(l,n+1),"xX")do u..="AaBbCcDdEeFf"n+=2elseif t=="0"and f(e(l,n+1),"bB")do u="01"n+=2end while(true)t=e(l,n)if t=="."and a do a=false elseif not f(t,u)do break end n+=1
r=sub(l,i,n-1)if(not tonum(r))d"bad number"r="0"
add(o,tonum(r))elseif z(t)do while(z(e(l,n)))n+=1
add(o,sub(l,i,n-1))elseif t=="'"or t=='"'do r,n=か(l,n+1,t,d)add(o,{t=r})elseif t=="["and f(e(l,n+1),"=[")do r,n=W(l,n,d,true)add(o,{t=r})else n+=1local e,r,u=unpack(split(sub(l,n,n+2),""))if e==t and r==t and f(t,".>")do n+=2if(u=="="and f(t,">"))n+=1
elseif e==t and r~=t and f(t,"<>")and f(r,"<>")do n+=2if(u=="=")n+=1
elseif e==t and f(t,".:^<>")do n+=1if(r=="="and f(t,".^<>"))n+=1
elseif e=="="and f(t,"+-*/\\%^&|<>=~!")do n+=1elseif f(t,"+-*/\\%^&|<>=~#(){}[];,?@$.:")do else d("bad char: "..t)end add(o,sub(l,i,n-1))end if(not a)add(h,s)add(b,i)add(p,n-1)
...
return n end local function Z(r,o,i)local n={}if i do add(n,i)elseif not e")"do while true do add(n,(t()))if(e")")break
...
end local function q()local r,d,c,a={},{},1while not e"}"do a=nil local i,f if e"["do i=t()l"]"l"="f=t()elseif o[n+1]=="="do i=g(u())l"="f=t()else i=g(c)f=t()c+=1a=#r+1end add(r,i)add(d,f)if(e"}")break
if(not e";")l","
...
end return t end end local function D(o,a)local n,p,t if o do if a do _()n=u()d[n]=f t=C(n)else n={u()}while(e".")add(n,u())
if(e":")add(n,u())p=true
...
l"("if not e")"do while true do if(e"...")r=true else add(n,u())
if(e")")break
...
add(e,l)local n=f(e)deli(e)if n do if(type(n)=="table")return c(n)
...
end local function j(d,e,a)local i=o[n]n+=1local r,f if a do if(i==".")r=u()return function(n)return e(n)[r]end,true,function(n)return e(n),r end
...
if i==":"do r=u()if(o[n]=="{"or type(o[n])=="table")f=B()return Z(e,r,f)
...
end local function E()local r,o,t,n=o[n]=="(",t()if e"then"do t,n=b()if e"else"do n=b()l"end"elseif e"elseif"do n=E()else l"end"end else C(r)t=b()if(not x()and e"else")n=b()
x=nil end return function(e)if o(e)do return t(e)elseif n do return n(e)end end end local function B(...)local n=y y=s+1local e=b(...)y=n return e end local function Z(n,e)if(n==true)return
...
while(#n>r)deli(n)
if(e)return Z(e,t)
until t end end local function I()if o[n+1]=="="do local r=u()l"="local o=t()l","local i,e=t(),e","and t()or g(1)l"do"_()d[r]=f local t=B()l"end"z()return function(n)for e=o(n),i(n),e(n)do if(stat(1)>=1)w()
...
if o[n]==";"or r(o[n],゜)or x and x()do return function()return pack()end else local n,r,l=t()local n={n}while(e",")add(n,(t()))
if#n==1and l and h do return function(n)local n,e=l(n)if(stat(1)>=1)w()
...
n-=1local r,t,f,l=n,t()if e","or e"="do n=r return j()elseif W(o[n])do return q(t,f)elseif s<=1and A do return function(n)local n=pack(t(n))if(not(l and n.n==0))add(a,n)
...
s-=1p=getmetatable(p).__index return function(e)local r,o,t,n=1,#l while r<=o do t,n=l[r](e)if t do if(type(t)~="number")break
if(n.e~=d)break
...
end return true end function B(e,t)local n=type(e)if n=="nil"do return"nil"elseif n=="boolean"do return e and"true"or"false"elseif n=="number"do return tostr(e,O)elseif n=="string"do return こ(e)elseif n=="table"and not t do local n,t,r="{",0,0for e,l in next,e do if(t==N)n=n..",<...>"break
if(t>0)n=n..","
local l=B(l,1)if e==r+1do n=n..l r=e elseif さ(e)do n=n..e.."="..l else n=n.."["..B(e,1).."]="..l end t+=1end return n.."}"else return"<"..tostr(n)..">"end end function し(n,e)if(e==nil)return n
if(not n)n=""
...
local t=i[5]if n==false do t=i[6]elseif n==true do t=i[7]elseif type(n)~="string"or r(n,{"nil","true","false"})do t=i[8]elseif q[n]do t=i[9]elseif not z(e(n,1))do t=i[10]elseif ▒[n]do t=i[11]end return t end)end function _draw()local u,c,p=peek(24357),peek2(24360),peek2(24362)camera()local function e(n)cursor(0,127)for n=1,n do rectfill(0,o*6,127,(o+1)*6-1,0)if(o<21)o+=1else print""
end end local function w(n,e)for n=1,n do if(o>e)o-=1
rectfill(0,o*6,127,(o+1)*6-1,0)end end local function m(n,e)for t=0,2do local l=pget(n+t,e+5)pset(n+t,e+5,l==0and i[12]or 0)end end local function f(f)local r=d..n.." "local l,t,n=X(r,#d+l)if n>s do e(n-s)elseif n<s do w(s-n,n)end s=n k=mid(k,0,max(s-21,0))::n::local n=o-s+k if(n+t<0)k+=1goto n
if(n+t>=21)k-=1goto n
...
if(n=="\r"or n=="\n")h+=t return true
end end end::n::local r,t if a or b do r,t=C(b,0,h)if(t-h<=20and a)b,a=し(b,a)r,t=C(b,0,h)if(#a==0and not x)a=nil
end if(not x)camera()
if(h==0and not x)f(not b)
if b do local u,r=sub(b,r),min(t-h,20)e(r)Q(u,0,(o-r)*6,i[1])if r<t-h do if(d(r))goto n
...
if G and not D do x="running, press 'esc' to abort"_draw()flip()x=nil else if(H and not D and not E)flip()
...
m=false return c(e)end t.stat=function(n,...)if n==30do return#F>0or stat(n,...)elseif n==31do if#F>0do return deli(F,1)else local n=stat(n,...)if(n=="•")Z=true
...
flip()E=true w()end D=false end function く(e)if r(e,{"i","interrupt"})do return G elseif r(e,{"f","flip"})do return H elseif r(e,{"r","repl"})do return A elseif r(e,{"mi","max_items"})do return N elseif r(e,{"h","hex"})do return O elseif r(e,{"cl","colors"})do return i elseif r(e,{"c","code"})do local n={[0]=n}for e=1,#u-1do n[e]=u[#u-e]end return n elseif r(e,{"cm","compile"})do return function(n)return そ(n)end elseif r(e,{"x","exec"})do return function(n,e)R(n,e)end elseif r(e,{"v","eval"})do return function(n,e)return S(n,e)end elseif r(e,{"p","print"})do return function(n,...)t.print(B(n),...)end elseif r(e,{"ts","tostr"})do return function(n)return B(n)end elseif r(e,{"rst","reset"})do run()elseif r(e,{"run"})do ち(t)else assert(false,"unknown \\-command")end end function け(e)local function t(n)return n and n~=0and true or false end local n if r(e,{"i","interrupt"})do n=function(n)G=t(n)end elseif r(e,{"f","flip"})do n=function(n)H=t(n)end elseif r(e,{"r","repl"})do n=function(n)A=t(n)end elseif r(e,{"mi","max_items"})do n=function(n)N=tonum(n)or-1end elseif r(e,{"h","hex"})do n=function(n)O=t(n)end elseif r(e,{"cl","colors"})do n=function(n)i=n end else assert(false,"unknown \\-command assign")end local n={__newindex=function(t,l,e)n(e)end}return setmetatable(n,n),0end L=stat(4)j,M=0,false poke(24412,10,2)function p(n)if stat(28,n)do if(n~=T)T,j=n,0
return j==0or j>=10and j%2==0elseif T==n do T=nil end end function _update()local t=false local function r(r)local t,e,o=X(d..n,#d+l)if(U)t=U
...
t=true end local function d()if#n>0do if(#u>50)del(u,u[1])
u[#u]=n add(u,"")y=#u t=true end end local function s(e)if(l+e>0)n=sub(n,1,l+e-1)..sub(n,l+e+1)l+=e t=true
end local function o(e)n=sub(n,1,l-1)..e..sub(n,l)l+=#e t=true end local i,h,e=stat(28,224)or stat(28,228),stat(28,225)or stat(28,229),-1if p(80)do if(l>1)l-=1t=true
elseif p(79)do if(l<=#n)l+=1t=true
elseif p(82)do if((i or not r(-1))and y>1)c(-1)
elseif p(81)do if((i or not r(1))and y<#u)c(1)
else local r=stat(31)e=ord(r)if r=="•"do if(#n==0)extcmd"pause"else a,I={}d()
elseif r=="\r"or r=="\n"do if(h)o"\n"else え(n)if(not a)o"\n"else d()
elseif i and p(40)do え(n,true)d()elseif r~=""and e>=32and e<154do if(M and e>=128)r=chr(e-63)
o(r)elseif e==193do o"\n"elseif e==192do f(-1)elseif e==196do f(1)elseif e==203do M=not M _,J="shift now selects "..(M and"punycase"or"symbols"),40elseif p(74)do if(i)l=1t=true else f(-1)
elseif p(77)do if(i)l=#n+1t=true else f(1)
elseif p(42)do s(-1)elseif p(76)do s(0)end end local r=stat(4)if(r~=L or e==213)o(r)L=r
if e==194or e==215do if n~=""and n~=L do L=n printh(n,"@clip")if(e==215)n=""l=1
...
if(t)v,U=20
j+=1い()end function お(n,e)local e,t=coresume(cocreate(e))if not e do printh("error #"..n..": "..t)print("error #"..n.."\npico8 broke something again,\nthis cart may not work.\npress any button to ignore")while(btnp()==0)flip()
